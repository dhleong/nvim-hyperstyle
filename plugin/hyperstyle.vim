" Lua-based vim-hyperstyle plugin
...
if !has('nvim') || !has('nvim-0.5')
  echohl WarningMsg
  echomsg "vim-hyperstyle (Lua version) requires Neovim 0.5+."
  echohl None
  finish
endif
...
function! s:map_key(key, binding)
  ...
  elseif oldmap =~# "<Plug>(".a:binding.")"
    " already mapped. maybe the user was playing with `set ft`
  elseif oldmap != ""
    exe "imap <silent> <buffer> ".a:key." ".oldmap."<Plug>(".a:binding.")"
  else
